...
files_modified: [src/orchestration/projection.py, src/infrastructure/projections.py]
autonomous: true
---

<objective>
Create WorkspaceProjection for reading workspace state from events.

Purpose: Enable loading/rebuilding workspace state from event stream.
Output: WorkspaceProjection that maintains workspace read model.
</objective>
...
<task type="auto">
  <name>Task 1: Create WorkspaceProjection</name>
  <files>src/orchestration/projection.py</files>
  <action>
Create WorkspaceProjection following the pattern from src/infrastructure/projections.py:
...
```
  ...
  <verify>python -c "from src.orchestration.projection import WorkspaceProjection; p = WorkspaceProjection(); print('Projection OK')"</verify>
  <done>WorkspaceProjection created with event handlers</done>
</task>

<task type="auto">
  <name>Task 2: Update orchestration __init__.py</name>
  <files>src/orchestration/__init__.py</files>
  <action>
Add WorkspaceProjection export:
...
Add to __all__ list.
  ...
  <verify>python -c "from src.orchestration import WorkspaceProjection"</verify>
  <done>WorkspaceProjection exported from package</done>
</task>
...
- [ ] WorkspaceProjection.apply() handles all Task events
- [ ] get_workspace() returns correct workspace
- [ ] get_or_create_workspace() creates new workspace if needed
- [ ] has_active_workspace() works for thread lookup
- [ ] get_active_workspace_threads() returns set of active threads
</verification>

<success_criteria>
- All tasks completed
- Projection can rebuild state from events
- Follows existing projection patterns
</success_criteria>

<output>
After completion, create `.planning/phases/05-process-orchestration/05-06-SUMMARY.md`
</output>
