...
files_modified: [src/graph/intent_stage2.py]
autonomous: true
---

<objective>
Create Stage 2 intent/TaskPlan extraction.

Purpose: Given mode from Stage 1, extract specific intent, targets, and optionally decompose into TaskPlan. This is the "detailed" stage with full context.
Output: Stage 2 classifier returning IntentEnvelope (single/plan/ambiguous).
</objective>
...
<task type="auto">
  <name>Task 1: Create Stage 2 intent classifier</name>
  <files>src/graph/intent_stage2.py</files>
  <action>
Create Stage 2 intent extraction following Phase 39 spec:
...
MODE_INTENTS: dict[SuperMode, list[Intent]] = {
    ...
    SuperMode.DECIDE: [Intent.DECISION_RECORD, Intent.DECISION_UPDATE, Intent.DECISION_LINK],
...
INTENT_RISK: dict[Intent, RiskLevel] = {
    ...
    Intent.DECISION_RECORD: RiskLevel.WRITE,
    Intent.DECISION_UPDATE: RiskLevel.WRITE,
    Intent.DECISION_LINK: RiskLevel.SAFE,
...
Stage 2 does the heavy lifting: full intent extraction, target parsing, TaskPlan decomposition.
  </action>
  <verify>python -c "from src.graph.intent_stage2 import classify_intent; print('OK')"</verify>
  <done>Stage 2 intent classifier with single/plan/ambiguous output</done>
</task>

<task type="auto">
  <name>Task 2: Add target extraction helpers</name>
  <files>src/graph/intent_stage2.py</files>
  <action>
Add helper functions for deterministic target extraction (before LLM):
...
This separates deterministic extraction from LLM classification per spec.
  </action>
  <verify>python -c "from src.graph.intent_stage2 import extract_jira_keys, build_target_hints; print(extract_jira_keys('Check SCRUM-123'))"</verify>
  <done>Deterministic target extraction helpers</done>
</task>
...
- [ ] python -m py_compile src/graph/intent_stage2.py
- [ ] classify_intent returns IntentEnvelope
- [ ] _build_single_envelope creates valid single envelope
- [ ] _build_plan_envelope sorts tasks by risk
- [ ] extract_jira_keys finds SCRUM-123 pattern
- [ ] Fallback to ambiguous on parse errors
</verification>

<success_criteria>
- Stage 2 classifier module created
- Returns IntentEnvelope (single/plan/ambiguous)
- Decomposes multi-intent into TaskPlan
- Sorts tasks by risk (safe first)
- Deterministic target extraction helpers
- Fallback to ambiguous on errors
</success_criteria>

<output>
After completion, create `.planning/phases/39-intent-classification-v2/39-04-SUMMARY.md`
</output>
