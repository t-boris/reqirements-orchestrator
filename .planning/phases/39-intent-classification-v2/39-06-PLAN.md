...
files_modified: [src/graph/intent_router.py]
autonomous: true
---

<objective>
Create unified intent router orchestrating all stages.

Purpose: Single entry point that runs Stage 0 → Stage 1 → Stage 2 → Policy, handling bypasses and returning final IntentEnvelope.
Output: intent_router_node for LangGraph integration.
</objective>
...
@src/graph/intent_policy.py

Current router (to replace):
@src/graph/intent.py
</context>
...
<task type="auto">
  <name>Task 1: Create unified intent router</name>
  <files>src/graph/intent_router.py</files>
  <action>
Create intent router orchestrating all stages per Phase 39 spec:
...
async def intent_router_node(state: "AgentState") -> dict[str, Any]:
    ...
    """
    from src.context.builder import build_context_packet
    ...
    # Build context
    context_packet = await build_context_packet(state)
    context_str = context_packet.to_prompt_string() if context_packet else ""

    # Route
    result = await route_intent(input, context=context_str)
...
Router orchestrates full pipeline with proper logging.
  </action>
  <verify>python -c "from src.graph.intent_router import route_intent, RouterInput; print('OK')"</verify>
  <done>Unified intent router with Stage 0→1→2→Policy pipeline</done>
</task>

<task type="auto">
  <name>Task 2: Add route_after_intent helper</name>
  <files>src/graph/intent_router.py</files>
  <action>
Add routing decision helper for LangGraph:
...
Routing logic for LangGraph conditional edges.
  ...
  <verify>python -c "from src.graph.intent_router import route_after_intent, is_terminal_intent; print('OK')"</verify>
  <done>Routing helpers for LangGraph integration</done>
</task>
...
- [ ] python -m py_compile src/graph/intent_router.py
- [ ] route_intent returns RouterResult
- [ ] Stage 0 bypass creates valid envelope
- [ ] Stage 0 constraints boost priority modes
- [ ] intent_router_node returns backward-compat intent_result
- [ ] route_after_intent routes correctly by kind
</verification>

<success_criteria>
- Unified intent router module created
- Full pipeline: Stage 0 → Stage 1 → Stage 2 → Policy
- RouterInput/RouterResult dataclasses
- _create_bypass_envelope for Stage 0 bypass
- _apply_constraint for Stage 0 constraints
- intent_router_node for LangGraph
- route_after_intent for conditional routing
- Backward compatibility with intent_result
</success_criteria>

<output>
After completion, create `.planning/phases/39-intent-classification-v2/39-06-SUMMARY.md`
</output>
