...
wave: 2
depends_on: ["38-02"]
files_modified: [src/context/message_index.py, src/context/__init__.py]
autonomous: true
---

<objective>
Create MessageIndex for caching rendered messages.

Purpose: Block rendering is expensive. Cache rendered messages to avoid re-computing on every request.
Output: MessageIndex class with LRU cache and message normalization.
</objective>
...
BlockRenderer (from 38-02):
@src/slack/block_renderer.py
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create NormalizedMessage model</name>
  <files>src/context/message_index.py</files>
  <action>
Create NormalizedMessage for consistent message representation:
...
```
  ...
  <verify>python -c "from src.context.message_index import NormalizedMessage; print('OK')"</verify>
  <done>NormalizedMessage dataclass created</done>
</task>

<task type="auto">
  <name>Task 2: Create MessageIndex class</name>
  <files>src/context/message_index.py</files>
  <action>
Add MessageIndex for caching and normalizing messages:
...
```
  ...
  <verify>python -c "from src.context.message_index import MessageIndex, normalize_message; print('OK')"</verify>
  <done>MessageIndex class with caching and normalization</done>
</task>
...
<task type="auto">
  ...
  <action>
Add MessageIndex exports to src/context/__init__.py:
...
```
  ...
  <verify>python -c "from src.context import MessageIndex, NormalizedMessage, normalize_message; print('OK')"</verify>
  <done>All context module exports updated</done>
</task>
...
- [ ] python -m py_compile src/context/message_index.py
- [ ] python -c "from src.context import MessageIndex, normalize_message"
- [ ] Test normalize() with user message returns message_type="user"
- [ ] Test normalize() with bot message renders blocks
- [ ] Test cache works (same message returns cached result)
</verification>

<success_criteria>
- NormalizedMessage with message_type, author, text, rendered_text
- MessageIndex with LRU caching
- normalize() handles user, bot, system messages
- Bot messages with blocks get rendered_text from BlockRenderer
- Module exports updated
</success_criteria>

<output>
After completion, create `.planning/phases/38-context-architecture/38-04-SUMMARY.md`
</output>
