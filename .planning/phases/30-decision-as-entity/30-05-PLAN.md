...
wave: 2
depends_on: ["30-01"]
files_modified: [src/slack/blocks/decision_cards.py]
autonomous: true
---

<objective>
Create Decision UI blocks with four visual states.

Purpose: Visual representation of decisions matching psychological weight — draft/proposal/approved/commit log.
Output: Decision card blocks for all four states with action buttons.
</objective>
...
<task type="auto">
  <name>Task 1: Create compact draft card (discussion mode)</name>
  <files>src/slack/blocks/decision_cards.py</files>
  <action>
Create src/slack/blocks/decision_cards.py with decision card blocks.

Start with the compact draft card — used during discussion when decision is PROPOSED:
...
```
  ...
  <verify>python -c "from src.slack.blocks.decision_cards import build_compact_draft_card; print('OK')"</verify>
  <done>Compact draft card for PROPOSED decisions</done>
</task>

<task type="auto">
  <name>Task 2: Create full approval block (heavy, formal)</name>
  <files>src/slack/blocks/decision_cards.py</files>
  <action>
Add full approval block for the "are you sure?" moment.
...
def build_approval_block(
    ...
    Format:
    ━━━━━━━━━━━━━━━━━━━━━━
    🧠 Decision DEC-{id} ({type}) — Ready for approval
...
```
  ...
  <verify>python -c "from src.slack.blocks.decision_cards import build_approval_block; print('OK')"</verify>
  <done>Full approval block for "are you sure?" moment</done>
</task>

<task type="auto">
  <name>Task 3: Create approved card and commit log entry</name>
  <files>src/slack/blocks/decision_cards.py</files>
  <action>
Add approved card (compact authoritative) and commit log entry (ultra compact).
...
def build_approved_card(
    ...
    Format:
    🧠 DEC-{id} v{version} ({type}) — Approved
    {title}
...
```
  ...
  <verify>python -c "from src.slack.blocks.decision_cards import build_approved_card, build_commit_log_entry, build_deprecated_decision_blocks; print('OK')"</verify>
  <done>Approved card, commit log entry, and deprecated blocks</done>
</task>
...
- [ ] build_compact_draft_card() for PROPOSED decisions (lightweight)
- [ ] build_approval_block() for approval moment (heavy, formal)
- [ ] build_approved_card() for APPROVED decisions (authoritative)
- [ ] build_commit_log_entry() for Channel Work Board (ultra compact)
- [ ] build_deprecated_decision_blocks() for deprecated decisions
- [ ] All buttons have version-bound payloads (decision_id + version)
...
<success_criteria>
- All tasks completed
- Four visual states match CONTEXT.md specification
- Version-bound buttons prevent stale clicks
- Same object, four visual identities: idea -> proposal -> law -> record
</success_criteria>

<output>
After completion, create `.planning/phases/30-decision-as-entity/30-05-SUMMARY.md`
</output>
