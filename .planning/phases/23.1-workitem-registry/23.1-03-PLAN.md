...
wave: 2
depends_on: [23.1-01]
files_modified: [tests/db/test_workitem_store.py]
autonomous: true
---

<objective>
Create tests for WorkItemStore CRUD operations.

Purpose: Verify WorkItemStore correctly persists and retrieves WorkItems.
Output: Test file with comprehensive coverage of all store methods.
</objective>
...
@src/db/models.py
@tests/db/test_session_store.py (for test patterns)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create WorkItemStore test file</name>
  <files>tests/db/test_workitem_store.py</files>
  <action>
Create test file following existing test patterns. If tests/db/ doesn't exist, create it.
...
Note: This test file assumes a `db_connection` fixture exists in conftest.py. If not present, the tests will need that fixture added separately.
  </action>
  <verify>python -m pytest tests/db/test_workitem_store.py --collect-only</verify>
  <done>Test file created with 15+ test cases covering all CRUD operations</done>
</task>
...
- [ ] Test file exists at tests/db/test_workitem_store.py
- [ ] `python -m pytest tests/db/test_workitem_store.py --collect-only` shows all tests
...
- Tests cover edge cases (nonexistent items, filters, hierarchy)
</success_criteria>

<output>
After completion, create `.planning/phases/23.1-workitem-registry/23.1-03-SUMMARY.md`
</output>
