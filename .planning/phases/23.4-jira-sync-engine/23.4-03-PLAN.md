...
files_modified: [src/jira/sync_service.py, src/jira/__init__.py]
autonomous: true
---

<objective>
Implement JiraSyncService for bidirectional sync with conflict detection.

Purpose: Core sync logic that respects field ownership and detects conflicts.
Output: JiraSyncService class with sync_to_jira() and sync_from_jira() methods.
</objective>
...
@.planning/phases/23.4-jira-sync-engine/23.4-01-SUMMARY.md
@.planning/phases/23.4-jira-sync-engine/23.4-02-SUMMARY.md

@src/jira/client.py (for JiraService)
@src/jira/sync_config.py (for field classification)
@src/jira/fingerprint.py (for description fingerprinting)
@src/db/workitem_store.py (for WorkItemStore)
...
<task type="auto">
  <name>Task 1: Create JiraSyncService</name>
  <files>src/jira/sync_service.py</files>
  <action>
Create new file for bidirectional sync service:
...
```
  ...
  <verify>python -c "from src.jira.sync_service import JiraSyncService; print('OK')"</verify>
  <done>JiraSyncService with bidirectional sync and conflict detection</done>
</task>
...
<task type="auto">
  ...
  <action>
Add exports for sync_service module.
...
Add to __all__ if present.
  ...
  <verify>python -c "from src.jira import JiraSyncService; print('OK')"</verify>
  <done>JiraSyncService exported from jira package</done>
</task>
...
- [ ] `python -c "from src.jira.sync_service import JiraSyncService"` succeeds
- [ ] `python -c "from src.jira import JiraSyncService"` succeeds
- [ ] No import errors
</verification>

<success_criteria>
- JiraSyncService with sync_to_jira() and sync_from_jira() methods
- Section-level conflict detection for description
- create_in_jira() for explicit creation with commit entry
- Auto-merge of non-conflicting changes
</success_criteria>

<output>
After completion, create `.planning/phases/23.4-jira-sync-engine/23.4-03-SUMMARY.md`
</output>
