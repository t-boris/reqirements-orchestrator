...
wave: 1
depends_on: []
files_modified: [src/jira/fingerprint.py]
autonomous: true
---

<objective>
Implement section-level fingerprinting for description sync.

Purpose: Enable detection of which description sections changed for granular conflict resolution.
Output: SectionFingerprint model and functions for computing/comparing fingerprints.
</objective>
...
@src/db/models.py (for WorkItem.jira_fingerprint field)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create fingerprint module</name>
  <files>src/jira/fingerprint.py</files>
  <action>
Create new file for section-level fingerprinting:
...
```
  ...
  <verify>python -c "from src.jira.fingerprint import compute_fingerprint, detect_conflict; print('OK')"</verify>
  <done>Fingerprint module with section parsing and conflict detection</done>
</task>
...
- [ ] `python -c "from src.jira.fingerprint import compute_fingerprint"` succeeds
- [ ] `python -c "from src.jira.fingerprint import parse_sections"` succeeds
- [ ] `python -c "from src.jira.fingerprint import detect_conflict"` succeeds
- [ ] No import errors
</verification>

<success_criteria>
- parse_sections() extracts standard sections from description
- compute_fingerprint() generates per-section hashes
- fingerprint_to_dict/dict_to_fingerprint for storage compatibility
- detect_conflict() identifies which sections have conflicts
</success_criteria>

<output>
After completion, create `.planning/phases/23.4-jira-sync-engine/23.4-02-SUMMARY.md`
</output>
