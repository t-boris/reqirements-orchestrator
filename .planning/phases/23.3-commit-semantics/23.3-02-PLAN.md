...
wave: 1
depends_on: []
files_modified: [src/slack/blocks/commit.py, src/slack/blocks/__init__.py]
autonomous: true
---

<objective>
Create commit preview UI blocks with Approve & Commit button.

Purpose: Show users what will be committed to channel truth and get explicit approval.
Output: build_commit_preview_blocks() function and related UI components.
</objective>
...
@src/slack/blocks/decisions.py (for block patterns)
@src/slack/blocks/draft.py (for block patterns)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create commit preview blocks module</name>
  <files>src/slack/blocks/commit.py</files>
  <action>
Create new file for commit preview UI:
...
```
  ...
  <verify>python -c "from src.slack.blocks.commit import build_commit_preview_blocks, build_commit_success_blocks; print('OK')"</verify>
  <done>Commit preview and success blocks with Approve & Commit button</done>
</task>

<task type="auto">
  <name>Task 2: Export from blocks module</name>
  <files>src/slack/blocks/__init__.py</files>
  <action>
Add exports to src/slack/blocks/__init__.py:
...
Add to __all__ if it exists.
  ...
  <verify>python -c "from src.slack.blocks import build_commit_preview_blocks; print('OK')"</verify>
  <done>Commit blocks exported from blocks module</done>
</task>
...
- [ ] `python -c "from src.slack.blocks.commit import build_commit_preview_blocks"` succeeds
- [ ] `python -c "from src.slack.blocks import build_commit_preview_blocks"` succeeds
- [ ] No import errors
</verification>

<success_criteria>
- build_commit_preview_blocks() creates proper Slack blocks
- Approve & Commit button with primary style
- Edit and Not now buttons included
- build_commit_success_blocks() for confirmation display
</success_criteria>

<output>
After completion, create `.planning/phases/23.3-commit-semantics/23.3-02-SUMMARY.md`
</output>
